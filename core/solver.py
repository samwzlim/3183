...
class Solver(nn.Module):
    def __init__(self, args):
        ...
        if args.mode == 'train':
            ...
            self.schedulers = Munch()
            for net in self.nets.keys():
                if net == 'fan':
                    continue
                self.schedulers[net] = ExponentialLR(self.optims[net], gamma=0.99)
    ...
    def compute_perceptual_loss(self, x_fake, x_real):
        features_fake = self.vgg(x_fake)
        features_real = self.vgg(x_real)
        loss_percep = F.l1_loss(features_fake, features_real)
        return loss_percep
    ...
    def compute_d_loss(self, nets, args, x_real, y_org, y_trg, z_trg=None, x_ref=None, masks=None):
        ...
        gradient_penalty = self.compute_gradient_penalty(nets.discriminator, x_real, x_fake.detach(), y_org)

        loss = loss_real + loss_fake + args.lambda_reg * loss_reg + args.lambda_gp * gradient_penalty
    ...
    def compute_g_loss(self, nets, args, x_real, y_org, y_trg, z_trgs=None, x_refs=None, masks=None):
        ...
        loss_percep = self.compute_perceptual_loss(x_fake, x_real)

        loss = loss_adv + args.lambda_sty * loss_sty - args.lambda_ds * loss_ds + args.lambda_cyc * loss_cyc + args.lambda_percep * loss_percep
    ...
    def compute_gradient_penalty(self, discriminator, real_samples, fake_samples, y_org):
        alpha = torch.rand(real_samples.size(0), 1, 1, 1).to(real_samples.device)
        interpolates = (alpha * real_samples + ((1 - alpha) * fake_samples)).requires_grad_(True)
        d_interpolates = discriminator(interpolates, y_org)
